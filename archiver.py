...
class FilenameParts(BaseModel):
    ...
    def __init__(self, filename: str):
        ...
        super().__init__(
            ...
            rawpart_enumeration=(
                int(m.group("rawpart")) if m.group("rawpart") is not None else None
            ),
...
def setupCSVLOG() -> None:
    ...
    if not os.path.isfile(CSVLOG):
        with open(CSVLOG, "w") as csvfile:
            ...
            csvwriter.writerow(
                ["timestamp", "obs_id", "archival_status", "notes", "data_path"]
            )
    else:
        pass
...
def checkForCalibrationProducts(calibration_dir: str) -> bool:
    ...
    if all(subdir in subdirs for subdir in ["fixed_delays", "fixed_phases"]):
        ...
        logger.info(
            f"Found calibration directory in '{root}', with all critical subdirectories. Calibration did occur."
        )
        return True
    else:
        logger.info(
            f"Found calibration directory in '{root}', but missing critical subdirectories. Skipping."
        )
        return False
...
def checkCalibrationGains(calibration_dir: str) -> bool:
    ...
    for subdir in subdirs:
        ...
        if subdir == "calibration_gains":
            ...
            if any(
                [
                    re.match(r".*\.json", file)
                    for file in os.listdir(os.path.join(calibration_dir, subdir))
                ]
            ):
                ...
                with open(fname, "r") as f:
                    try:
                        ...
                        logger.error(
                            f"Error reading calibration gains from file {fname}."
                        )
                        return False
                    if "ref_ant" not in gains[list(gains.keys())[0]]:
                        logger.info(
                            f"Found calibration gains in '{root}', but no reference antenna field."
                        )
                        return False
                    else:
                        logger.info(
                            f"Found calibration gains in '{root}', with reference antenna field."
                        )
                        return True
            else:
                # We have a calibration gains folder but no gains:
                logger.info(
                    f"Found calibration_gains folder in '{root}', but no calibration gains therein."
                )
                return False
        else:
            continue
...
def determineReferenceAntenna(calibration_dir: str, obs_info: dict) -> str:
    ...
    json_file = os.path.join(
        calibration_dir,
        f"fixed_phases/{os.listdir(os.path.join(calibration_dir,'fixed_phases/'))[0]}",
    )
...
def fetchUVH5detail(uvh5_ac: str, uvh5_bd: str) -> (dict, bool):
    ...
    try:
        ...
        obs_info["start_epoch_seconds"] = Time(
            f"{uv.time_array[0] - 2400000.5}", format="mjd"
        ).unix
        obs_info["sideband_ac"] = (
            -1
            if (
                uv.extra_keywords["FirstChannelFrequencyHz"] < 12000
                and uv.extra_keywords["FirstChannelFrequencyHz"] > 8000
            )
            else 1
        )
        if obs_info["sideband_ac"] == -1:
            obs_info["fcentmhz_ac"] = (
                uv.extra_keywords["FirstChannelFrequencyHz"]
                - (uv.extra_keywords["NumberOfFEngineChannels"] // 2)
                * uv.extra_keywords["ChannelBandwidthHz"]
            ) * 1e-6
        else:
            obs_info["fcentmhz_ac"] = (
                uv.extra_keywords["FirstChannelFrequencyHz"]
        ...
        obs_info["observed_antenna"] = [
            uv.antenna_names[uv.antenna_numbers.tolist().index(antnum)]
            for antnum in ants_numbers
        ]

    except Exception as e:
        logger.error(
            f"Could not read uvh5 file '{uvh5_ac}' or extract required information: {e}"
        )
        return (
            f"Could not read uvh5 file '{uvh5_ac}' or extract required information: {e}",
            False,
    ...
    try:
        ...
        logger.error(
            f"Could not read uvh5 file '{uvh5_bd}' or extract required information: {e}"
        )
        return (
            f"Could not read uvh5 file '{uvh5_bd}' or extract required information: {e}",
            False,
...
def writeCalibrationCollationCommand(
    obs_info: dict, fcent_offset: float = 0.0
) -> (str, bool):
    ...
    if (
        obs_info["start_epoch_seconds"] > 1685810556.0
    ):  # we've already recorded gains from this point onwards, so ignore.
        logger.info(f"Scan should have already been archived. Skipping...")
        return (
            f"Scan is newer than {time.ctime(1685810556.0)} and should have already been archived. Skipping...",
            False,
        )
    ...
    with open(CSVLOG, "a") as csvfile:
        ...
        for root, dirs, files in os.walk(os.path.abspath(args.rootdir)):
            ...
            if "calibration" in dirs:
                ...
                if cal_products:
                    ...
                    csvwriter.writerow(
                        [
                            ...
                            "did not calibrate",
                            False,
                            "Directory contains no calibration products - therefore unsure calibration was ever run.",
                            root,
                        ]
                    )
                    continue
                ...
                if any([re.match(r".*\.uvh5", file) for file in files]):
                    ...
                    if len(observation_name) == 0:
                        logger.error(
                            f"UVH5 files in directory '{root}' are not uniform or parseable."
                        )
                        csvwriter.writerow(
                            [
                                timestamp,
                                "corrupt_uvh5",
                                False,
                                "UVH5 files in directory are not uniform.",
                                root,
                            ]
                        )
                        continue
                        ...
                        logger.info(
                            f"UVH5 files in directory '{root}' are uniform and parseable."
                        )
                        ...
                        if ac_file is None or bd_file is None:
                            logger.error(
                                f"Could not find uvh5 files for both tunings in '{root}'."
                            )
                            csvwriter.writerow(
                                [
                                    timestamp,
                                    observation_name,
                                    False,
                                    "Could not find at least 1 UVH5 file for both tunings.",
                                    root,
                                ]
                            )
                            continue
                        ...
                        if status == False:
                            logger.warning(
                                f"Error extracting observation information from: {ac_file} and {bd_file}"
                            )
                            csvwriter.writerow(
                                [timestamp, observation_name, False, obs_info, root]
                            )
                            continue

                        obs_info["reference_antenna"] = determineReferenceAntenna(
                            calibration_dir, obs_info
                        )

                        collate_command, status = writeCalibrationCollationCommand(
                            obs_info
                        )
                        if status == False:
                            logger.warning(
                                f"Error generating collation command: {collate_command}"
                            )
                            csvwriter.writerow(
                                [
                                    timestamp,
                        ...
                        if cal_products:
                            ...
                            if not have_gains:
                                ...
                                logger.debug(
                                    f"Found calibration products in '{root}', but no calibration gains, re-deriving gains."
                                )
                                uvh5_command, _ = writeUVH5CalibrationCommand(obs_info)

                                logger.debug(f"Executing:\n{uvh5_command}")
                                uvh5_process = subprocess.Popen(
                                    uvh5_command, shell=True
                                )
                                ...
                                if exit_code != 0:
                                    ...
                                    csvwriter.writerow(
                                        [
                                            ...
                                            observation_name,
                                            False,
                                            "Error recreating gains, unable to proceed",
                                            root,
                                        ]
                                    )
                                    continue
                                    ...
                                    if not have_gains:
                                        logger.error(
                                            f"Re-derivation of calibration gains failed."
                                        )
                                        csvwriter.writerow(
                                            [
                                                timestamp,
                                                observation_name,
                                                False,
                                                "Re-derivation of calibration gains failed. It is possibly a permissions or disk space error in writing out gains",
                                                root,
                                            ]
                                        )
                                        continue
                                ...
                                logger.info(
                                    f"Found calibration gains in '{root}', with reference antenna field. Not re-deriving."
                                )

                            # By this point we have calibration products and calibration gains, proceed with collation:
                            logger.debug(
                                f"Re-collating for retroarchival. Executing:\n{collate_command}"
                            )
                            collate_process = subprocess.Popen(
                                collate_command, shell=True, env=env
                            )
                            ...
                            if exit_code != 0:
                                ...
                                for fcent_offset in fcent_offset_to_try:
                                    ...
                                    collate_command, status = (
                                        writeCalibrationCollationCommand(
                                            obs_info, fcent_offset=fcent_offset
                                        )
                                    )
                                    if status == False:
                                        logger.warning(
                            ...
                            if exit_code != 0:
                                ...
                                csvwriter.writerow(
                                    [
                                        ...
                                        observation_name,
                                        False,
                                        f"Could not collate gains and calculate grade. This is likely due to observation not being present in Observation database",
                                        root,
                                    ]
                                )
                                continue
                            ...
                            csvwriter.writerow(
                                [
                                    ...
                                    observation_name,
                                    True,
                                    "Calibration grade archival successful",
                                    root,
                                ]
                            )
                            ...
                            logger.warning(
                                f"Found UVH5 files in '{root}', but no calibration products."
                            )
                            csvwriter.writerow(
                                [
                                    timestamp,
                                    observation_name,
                                    False,
                                    "Found UVH5 files in '{root}', but no calibration products.",
                                    root,
                                ]
                            )
                    ...
                    csvwriter.writerow(
                        [
                            ...
                            "no_uvh5",
                            False,
                            "No UVH5 files found in directory.",
                            root,
                        ]
                    )
                    continue
            ...
            # see if it is not to be processed or contains uvh5's
                ...
                if any([re.match(r".*\.uvh5", file) for file in files]):
                    logger.warning(
                        f"No calibration directory found in '{root}', but does contain uvh5 files. Skipping."
                    )
                    csvwriter.writerow(
                        [
                            timestamp,
                            "no calibration",
                            False,
                            "No calibration directory found but does contain uvh5's.",
                            root,
                        ]
                    )
                    continue
                    ...
                    logger.info(
                        f"Skipping '{root}' as it does not contain calibration products or uvh5 files."
                    )
                    continue
